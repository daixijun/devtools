...
export class GlobalShortcutManager {
  ...
  async initialize() {
    ...
    try {
      ...
      if (result?.data?.enabled && result?.data?.hotkey) {
        ...
        if (modifier === 'option') {
          ...
          accelerator = `${modifier.charAt(0).toUpperCase() + modifier.slice(1)}+${key}`
        }
  ...
  private async registerShortcut(accelerator: string, retryCount = 0) {
    try {
      ...
      if (errorMessage.includes('permission') || errorMessage.includes('accessibility')) {
        console.warn('全局快捷键权限问题：请在系统偏好设置 > 安全性与隐私 > 辅助功能中授权应用')
        return
      }
          ...
          if (alternatives.length > 0) {
            for (const alternative of alternatives) {
              try {
                ...
                console.log(`快捷键 ${accelerator} 被占用，已自动切换到 ${alternative}`)
                return
              } catch (altError) {
                continue
              }
