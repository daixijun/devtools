...
export const RegexTester: React.FC = () => {
  ...
  const [engine, setEngine] = useState<'rust' | 're2' | 'pcre' | 'golang' | 'javascript'>('rust');
  ...
  const [replaceResult, setReplaceResult] = useState<RegexReplaceResult | null>(null);
  ...
  // 更新高亮文本
  useEffect(() => {
    if (mode === 'test' && testResult && testResult.is_valid && testResult.matches.length > 0) {
  ...
  useEffect(() => {
    const validatePattern = async () => {
      ...
      try {
        const response = await invoke<{ success: boolean; data?: boolean; error?: string }>('validate_regex', {
          pattern: debouncedPattern,
          engine: engine
        });

        if (response.success && response.data !== undefined) {
          setIsValidPattern(response.data);
          setError('');
        } else {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      const response = await invoke<{ success: boolean; data?: RegexTestResult; error?: string }>('test_regex', {
        pattern: pattern,
        text: testText,
        flags: flags,
        engine: engine
      ...
      if (response.success && response.data) {
        ...
        if (!response.data.is_valid && response.data.error_message) {
  ...
  const handleReplace = useCallback(async () => {
    ...
    try {
      const response = await invoke<{ success: boolean; data?: RegexReplaceResult; error?: string }>('replace_regex', {
        pattern: pattern,
        text: testText,
  ...
  const highlightMatches = (text: string, matches: RegexMatch[]) => {
    ...
    matches.forEach((match, index) => {
      ...
      highlighted += `<mark class="bg-yellow-200 dark:bg-yellow-800" title="匹配 ${index + 1}: ${match.start}-${match.end}">${match.full_match}</mark>`;
      lastEnd = match.end;
    });
    ...
    if (!testResult.is_valid) {
      return (
        <div className="p-4 border rounded-lg bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800">
          <p className="text-red-600 dark:text-red-400 font-medium">正则表达式无效</p>
          {testResult.error_message && (
            <p className="text-red-500 dark:text-red-300 text-sm mt-1">{testResult.error_message}</p>
          )}
        </div>
      );
    }
      ...
      <div className="space-y-4 h-full flex flex-col">
        {testResult.matches.length > 0 && (
          <div className="space-y-2 h-full flex flex-col">
              <h4 className="font-medium text-gray-700 dark:text-gray-300">匹配详情:</h4>
              <div className="h-full overflow-y-auto border rounded-lg bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600">
                {testResult.matches.map((match, index) => (
                  <div key={index} className="p-3 border-b last:border-b-0 border-gray-100 dark:border-gray-700">
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <span className="font-medium text-gray-600 dark:text-gray-400">匹配 {index + 1}:</span>
                        <span className="ml-2 font-mono text-gray-900 dark:text-gray-100">{match.full_match}</span>
                      </div>
                      <div>
                        <span className="font-medium text-gray-600 dark:text-gray-400">位置:</span>
                        <span className="ml-2 text-gray-900 dark:text-gray-100">{match.start}-{match.end}</span>
                      </div>
                    </div>
                    
                    {match.groups.length > 0 && (
                      <div className="mt-2">
                        <span className="font-medium text-gray-600 dark:text-gray-400">捕获组:</span>
                        <div className="ml-4 space-y-1">
                          {match.groups.map((group, groupIndex) => (
                            <div key={groupIndex} className="text-sm">
                              <span className="text-gray-500 dark:text-gray-400">组 {groupIndex + 1}:</span>
                              <span className="ml-2 font-mono text-gray-900 dark:text-gray-100">{group || '(未匹配)'}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {Object.keys(match.named_groups).length > 0 && (
                      <div className="mt-2">
                        <span className="font-medium text-gray-600 dark:text-gray-400">命名组:</span>
                        <div className="ml-4 space-y-1">
                          {Object.entries(match.named_groups).map(([name, value]) => (
                            <div key={name} className="text-sm">
                              <span className="text-gray-500 dark:text-gray-400">{name}:</span>
                              <span className="ml-2 font-mono text-gray-900 dark:text-gray-100">{value || '(未匹配)'}</span>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        {testResult.matches.length === 0 && (
          <div className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-600">
            <p className="text-gray-600 dark:text-gray-400 text-center">没有找到匹配项</p>
          </div>
        )}
      </div>
    );
      ...
      return (
        <div className="p-4 border rounded-lg bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800">
          ...
          {replaceResult.error_message && (
            <p className="text-red-500 dark:text-red-300 text-sm mt-1">{replaceResult.error_message}</p>
          )}
        </div>
      );
    }
      ...
      <div className="space-y-4">
        ...
        {replaceResult.result && (
          <div className="space-y-2">
            <h4 className="font-medium text-gray-700 dark:text-gray-300">替换结果:</h4>
  ...
  const leftContent = (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex-shrink-0">
        <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-4">配置与控制</h3>
            ...
            <div className="flex gap-4">
              <label className="flex items-center text-gray-700 dark:text-gray-300">
                <input
                  ...
                  value="test"
                  checked={mode === 'test'}
                  onChange={(e) => setMode(e.target.value as 'test' | 'replace')}
                  className="mr-2"
                />
                测试匹配
              </label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                正则表达式
                {isValidPattern === true && (
                  <span className="ml-2 text-green-600 text-xs">✓ 语法正确</span>
                )}
            ...
            <InputField
              ...
              placeholder="输入正则表达式..."
              className={`font-mono ${
                isValidPattern === false ? 'border-red-300 dark:border-red-600' : ''
              }`}
            />
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-4">测试文本</h3>
